...
| **Nave acosadora que suelta un ITEM** | ✅ Implementado | Nave que dispara y molesta pero **no intenta matarte**, y se va sola pasado un tiempo. Si la matas, suelta un item. La decisión "¿la persigo y me expongo, o la dejo ir?" es lo que la hace buena — es el OVNI rojo de Space Invaders. Barato: un behavior + un temporizador de fuga + un drop |
| **Nave rival con misiles** | ✅ Implementado (t=58, a mitad de stage) | Un mini-jefe. Arquitectónicamente barato (el sistema del jefe ya existe, sería uno con menos fases). **Pero no va justo antes del jefe**: dos set-pieces pegados le quitan impacto al jefe. Va a mitad del stage → oleadas → rival → oleadas → enjambre → jefe |
| **Items persistentes entre partidas (hangar)** | ✅ Implementado (v1) | Ver `docs/HANGAR.md`. Sigue pendiente el choque con el multijugador: `localStorage` es editable y dos jugadores entran con naves desiguales |
| **Los niveles como un viaje** | 🔜 Es la identidad que se está buscando | No entornos sueltos, sino una **ruta continua**: empiezas cerca de tierra viendo el horizonte → subes a las nubes → más arriba → espacio → te acercas a otro planeta → lo exploras. Cada nivel es un tramo del viaje, así el juego cuenta algo sin necesidad de texto. Es más trabajo de arte, pero es lo que convierte una sucesión de fondos en una travesía. El orden importa: el fondo debe dejar claro **dónde estás en el trayecto** |
| **Entornos con identidad** | 🔜 Parte de lo anterior | Hoy todo pasa en el espacio. La idea es llegar a sitios reconocibles — cueva, océano, volcán, hielo — para que el jugador sepa dónde está de un vistazo. **Es también el vehículo natural para meter obstáculos**, que hoy no existen: cada entorno trae los suyos (estalactitas, corrientes, géiseres). Pide: capas de fondo con parallax + tilesets de obstáculos + colisión contra terreno, que es lo único realmente nuevo a nivel de motor |
| **Piezas de nave con trade-offs** | 🔜 Junto con lo anterior | Que las mejoras del hangar no sean solo "+X mejor", sino decisiones: disparo más fuerte pero más lento, más velocidad pero menos daño. Es lo que convierte el hangar en un sistema de build en vez de una lista de compras. **Requisito de arte ya cubierto**: el `ASSET_BRIEF.md` pide la nave por capas (casco/alas/motor/cañón) justo para poder intercambiar piezas sin redibujar |
| **Que la nave se vea distinta al equipar** | 🔜 Junto con lo anterior | Dos caminos posibles: equipar al instante durante la partida, o comprar piezas en el hangar. **No son excluyentes** y la recomendación es hacer los dos en capas distintas: el hangar define la identidad visual permanente de tu nave, y el medidor de poder (in-run) cambia el aspecto del arma. Cada uno da feedback en su escala de tiempo |
